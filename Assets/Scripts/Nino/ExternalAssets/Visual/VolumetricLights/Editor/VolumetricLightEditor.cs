...
namespace VolumetricLights {
    ...
    public partial class VolumetricLightEditor : Editor {
        ...
        SerializedProperty blendMode, raymarchPreset, raymarchQuality, raymarchMinStep, raymarchMaxSteps, dithering, jittering, useBlueNoise, animatedBlueNoise, renderQueue, sortingLayerID, sortingOrder, alwaysOn;
        ...
        SerializedProperty shadowBakeMode, shadowUseDefaultRTFormat, shadowOptimizeShadowCasters, shadowOrientation, shadowDirection;
        ...
        void OnEnable() {
            ...
            shadowUseDefaultRTFormat = serializedObject.FindProperty("shadowUseDefaultRTFormat");
            shadowOptimizeShadowCasters = serializedObject.FindProperty("shadowOptimizeShadowCasters");
        ...
        public override void OnInspectorGUI() {
            ...
            if (GUILayout.Button("Show Global Settings")) {
                VolumetricLightsPostProcessBase post = FindObjectOfType<VolumetricLightsPostProcessBase>();
                if (post == null) {
                    if (EditorUtility.DisplayDialog("Volumetric Lights Global Settings", "The post processing component of Volumetric Lights can't be found on the camera. Do you want to add it to the main camera?", "Yes", "No")) {
                        Camera cam = Camera.main;
                        if (cam == null) {
                            EditorUtility.DisplayDialog("Volumetric Lights Global Settings", "No main camera found.", "Ok");
                        } else {
                            post = cam.gameObject.AddComponent<VolumetricLightsPostProcess>();
                        }
                    }
                }
                if (post != null) {
                    Selection.SetActiveObjectWithContext(post, null);
                    GUIUtility.ExitGUI();
                }
            }
            ...
            if (castDirectLight.boolValue) {
                EditorGUI.indentLevel++;
                Camera cam = Camera.main;
                if (cam != null && cam.renderingPath != RenderingPath.DeferredShading) {
                    EditorGUILayout.HelpBox("Please note that Cast Direct Light option works better in deferred rendering mode (check your camera settings).", MessageType.Info);
                }
            ...
            if (enableShadows.boolValue) {
                ...
                EditorGUILayout.PropertyField(shadowUseDefaultRTFormat, new GUIContent("Use Default RT Format"));
                EditorGUILayout.PropertyField(shadowOptimizeShadowCasters, new GUIContent("Optimize Shadow Casters"));
                EditorGUI.indentLevel--;
            }
